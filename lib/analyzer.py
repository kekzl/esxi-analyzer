...
class IssueAnalyzer:
    ...
    def _read_file(self, filename: str) -> str:
        ...
        """
        file_path = Path(self.log_path) / filename
        if file_path.exists():
            try:
                ...
                if version_num < 6.5:
                    self.issues.append(
                        Issue(
                            ...
                            description=(
                                f"ESXi version {version} has reached end of life "
                                "and is no longer receiving security updates"
                            ),
    ...
    def _analyze_performance(self) -> None:
        ...
        if "%PCPU" in cpu_stats:
            ...
            high_cpu_processes = [
                f"{proc}: {usage}% CPU" for proc, usage in high_cpu_matches if float(usage) > high_cpu_threshold
            ]
                ...
                self.issues.append(
                    Issue(
                        ...
                        solution=(
                            ...
                            "and consider optimizing workloads or adding resources"
                        ),
    ...
    def _analyze_network(self):
        ...
        # vmkernel data reserved for future network analysis
    ...
    def _analyze_vms(self):
        ...
        # vm_stats reserved for future VM performance analysis
    ...
    def _analyze_logs(self):
        ...
        log_dir = Path(self.log_path) / "logs"
        ...
        error_patterns = {
            ...
            r"out\s+of\s+memory": {
                ...
                "description": (
                    "The ESXi host is experiencing memory pressure and may be running out of available memory"
                ),
                "category": Issue.CATEGORY_MEMORY,
        ...
        for log_file_path in log_dir.iterdir():
            try:
                ...
                for pattern, issue_info in error_patterns.items():
                    ...
                    if matches:
                        evidence = [f"{log_file_path.name}: {len(matches)} occurrences of pattern '{pattern}'"]
                        ...
                        self.issues.append(
                            Issue(
                                ...
                                description=str(issue_info["description"]),
                                category=str(issue_info["category"]),
                                severity=str(issue_info["severity"]),
                                evidence=evidence,
                                solution=str(issue_info["solution"]),
                                doc_links=list(issue_info["doc_links"]),
                            )
                        )

            except Exception as e:
                if self.verbose:
                    print(f"Error analyzing log file {log_file_path.name}: {e!s}")
